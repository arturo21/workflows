angular
	...
	.factory('sessele', session)
	session.$inject=['$http','$mdDialog','$window','$location','$route','$cookies'];
	function session($http,$mdDialog,$window,$location,$route,$cookies,$mdToast){
		var i;
		var valores;
		var res;
		var logusr;
		...
		function sanitizePosition() {
	  	...
	  	function showSimpleToast(mensaje) {
					...
					if(parseInt(respuesta[0].activado)==1){
					    ...
					    $mdDialog.show(
					      $mdDialog.alert()
					        .parent(angular.element(document.querySelector('body')))
					        .clickOutsideToClose(true)
					        .title('Usuario/Clave incorrecto!')
